...
class ImageGeometry:
    ...
    def from_image(cls, image: ImageData) -> 'ImageGeometry':
        ...
        return cls(
            dim_x=dims[2], dim_y=dims[1], dim_z=dims[0],
            voxel_x=voxels[2], voxel_y=voxels[1], voxel_z=voxels[0]
        )
...
class RotationParameters:
    ...
    def round_rotation_angle(self) -> None:
        ...
        if self.rotation_angle % 360 < 1e-2:
            ...
            raise ValidationError("Rotation angle must be a multiple of 180 or 360 degrees")
    ...
    def to_simind_params(self) -> Tuple[int, float]:
        ...
        if key not in switch_map:
            raise ValidationError(f"Unsupported rotation: {self.direction.value} {self.rotation_angle}°")
...
class ImageValidator:
    ...
    def validate_compatibility(image1: ImageData, image2: ImageData) -> None:
        ...
        if (geom1.voxel_x, geom1.voxel_y, geom1.voxel_z) != (geom2.voxel_x, geom2.voxel_y, geom2.voxel_z):
            raise ValidationError("Images must have same voxel sizes")
        
        if (geom1.dim_x, geom1.dim_y, geom1.dim_z) != (geom2.dim_x, geom2.dim_y, geom2.dim_z):
...
class GeometryManager:
    ...
    def configure_source_geometry(self, geometry: ImageGeometry) -> None:
        ...
        self.logger.info(f"Source geometry: {geometry.dim_x}×{geometry.dim_y}×{geometry.dim_z}")
...
class AcquisitionManager:
    ...
    def configure_rotation(self, rotation: RotationParameters, detector_distance: float) -> None:
        ...
        self.config.set_value(12, detector_distance / SIMIND_VOXEL_UNIT_CONVERSION)  # convert to cm
        self.config.set_value(30, rotation_switch)
        self.config.set_value(41, start_angle)
        
        self.logger.info(f"Rotation: {rotation.direction.value} {rotation.rotation_angle}° from {rotation.start_angle}°")
    
    def configure_energy_windows(self, windows: List[EnergyWindow], output_prefix: str) -> None:
...
class DataFileManager:
    ...
    def prepare_attenuation_file(self, mu_map: ImageData, output_prefix: str, 
                                use_attenuation: bool, photon_energy: float, 
                                input_dir: Path) -> str:
        ...
        if use_attenuation:
            ...
            mu_map_arr = attenuation_to_density(mu_map_arr, photon_energy, input_dir) * 1000
        else:
...
class OrbitFileManager:
    ...
    def write_orbit_file(self, radii: List[float], output_prefix: str, 
                        center_of_rotation: Optional[float] = None) -> Path:
        ...
        with open(orbit_file, 'r') as f:
            for line in f:
                ...
                if parts:
                    ...
                    radii.append(radius_cm * SIMIND_VOXEL_UNIT_CONVERSION)  # Convert to mm
        return radii
...
class SimindExecutor:
    ...
    def run_simulation(self, output_prefix: str, orbit_file: Optional[Path] = None,
                      runtime_switches: Optional[Dict] = None) -> None:
...
class OutputProcessor:
    ...
    def _apply_template_corrections(self, h00_file: Path, template_sinogram: AcquisitionData) -> None:
        ...
        try:
            ...
            if "number_of_projections" in attributes and "image_duration" in attributes:
                time_per_projection = attributes["image_duration"] / attributes["number_of_projections"]
                total_duration = attributes["number_of_projections"] * time_per_projection
            ...
            if "height_to_detector_surface" in attributes:
                expected_radius = attributes["height_to_detector_surface"]  # Already in mm from STIR
                current_radius = self.converter.read_parameter(str(h00_file), ";# Radius")
                
                if (current_radius is None or 
                    abs(float(current_radius) - expected_radius) > 0.1):
                    self.logger.info(f"Correcting radius from template: {expected_radius:.4f} mm")
                    self.converter.edit_parameter(
                        str(h00_file),
                        ";#Radius",
                        expected_radius
            ...
            self.logger.warning(f"Failed to apply template corrections to {h00_file}: {e}")
